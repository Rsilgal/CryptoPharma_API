...
const getOneProduct = async (token: getproductInterface) => {
    const response = await contract.methods.getProduct(token.tokenIdentifier).call();
    return response;
};
...
const createOneProduct = async (_product: createProductInterface) => {
    ...
    const response = await contract.methods.createProduct(name, description, lot, isPharmaService, isHospitalService, needAuthorization, quantity, expireDate, price, quantity).send({ from: controllerAddress });
...
const buyOneProduct = async (_product: buyProductInterface) => {
    ...
    const response = (await contract.methods.buyProduct(seller, buyer, productIdentifier, amount, prescriptionIdentifier).send({ from: buyer })); //, gas: '1000000', gasPrice: '10000000000'
...
const sellOneProduct = async (_product: sellProductInterface) => {
    ...
    const response = await contract.methods.sellProduct(seller, buyer, productIdentifuer, amount).send({ from: buyer });
